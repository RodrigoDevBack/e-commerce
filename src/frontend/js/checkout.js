...
export default function checkoutPage() {
  ...
  cart.forEach(item => {
    ...
    // Determina o preço (pode vir de item.price ou item.unity_price dependendo da estrutura)
    ...
    if (item.product?.images && item.product.images.length > 0) {
      thumbHTML = `<img src="http://api.singlotown.com.br/images_products/${item.product.name}/${item.product.images[0]}" alt="${productName}" class="checkout-item-thumb">`;
    } else {
...
export async function initCheckout() {
  ...
  if (response.success == false) {
    ...
    // Evento do botão "Cadastrar Endereço"
    if (saveAddressBtn) {
  // copy visual style from finalize button so both look the same
  try { if (finalizeBtnEl) saveAddressBtn.className = finalizeBtnEl.className; } catch(e){}
      ...
      saveAddressBtn.addEventListener('click', async (e) => {
        ...
        if (result.success) {
          ...
          try { saveAddressBtn.style.display = 'none'; } catch(e){}
        } else {
    ...
    // response is an address object returned from backend; show it and hide save button
    ...
    if (saveAddressBtn) {
      try { saveAddressBtn.style.display = 'none'; if (finalizeBtnEl) saveAddressBtn.className = finalizeBtnEl.className; } catch(e){}
    }
  }

  // Evento do botão "Concluir Compra"
  ...
  if (finalizeBtnEl && checkoutForm) {
    checkoutForm.addEventListener('submit', async (e) => {
      ...
      try {
        // Envia o pedido para o backend via proxy PHP
