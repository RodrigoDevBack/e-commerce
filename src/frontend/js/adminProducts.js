...
export async function initAdminProducts() {
  ...
  JSON.parse(products).forEach((product) => {
    ...
    novoProduto.innerHTML = `<td>${product.id}</td>
        <td>${product.name}</td>
...
function createEditProductModal(product) {
      ...
      <div class="modal fade" id="modal-edit-product-${
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="product-modal-label-${
                      product.id
                    }">
                        ${product.name}
                    </h1>
  ...
  const allCheckboxToRemoveImage = modalContainer.querySelectorAll(
    "#checkbox-remover-imagem"
  );
    ...
    checkbox.addEventListener("change", function () {
      ...
      if (checkbox.checked) {
        ...
        if (countFieldsToImages >= limitImages) {
          alert(
            `Atenção! Limite de imagens excedido em ${
              countFieldsToImages - limitImages
            }`
          );
        }
      } else {
        // Se inativar a checkbox a url é removida da lista
        limitImages--;
        urlImagesForRemove = urlImagesForRemove.filter(
  ...
  const btnRemoveFieldImage = modalContainer.querySelector(
    "#remove-campo-imagem"
  );
  ...
  const checkboxEditarNome = modalContainer.querySelector(
    "#checkbox-editar-nome"
  );
  const checkboxEditarDescricao = modalContainer.querySelector(
    "#checkbox-editar-descricao"
  );
  const checkboxEditarQtd = modalContainer.querySelector(
    "#checkbox-editar-qtd"
  );
  const checkboxEditarValorProduto = modalContainer.querySelector(
    "#checkbox-editar-valor-produto"
  );
  ...
  btnEditProduct.addEventListener("submit", async function (e) {
    ...
    let response = await requestEditProduct(
      product.id,
      name,
      description,
      value,
