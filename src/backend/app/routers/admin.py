...
admin = APIRouter(tags=["Admin"], responses={404: {"Description": "Not found"}})
...
async def disable_client_account(
    user: AdminDisableUserSchema, depends: Annotated[str, Depends(get_user_admin)]
):
    """...
...
async def add_product_image(
    id: int = Form(...), image: UploadFile = File(...), depends=Depends(get_user_admin)
):
    """...
...
async def create_product(
    data: AdminCreateProductSchema, depends=Depends(get_user_admin)
):
...
async def edit_product(data: AdminUpdateProductSchema, depends=Depends(get_user_admin)):
...
async def delete_product_image(
    id: int = Form(...), del_image: str = Form(...), depends=Depends(get_user_admin)
):
...
async def delete_product(
    ...
    await ProductService.delete_product(product_exists)
